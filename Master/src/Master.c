...
void hilo_reduccion_local(void* parametros){
	...
	//Serializacion
	int longitudIdWorker=strlen(estructura->idWorker)+1;
	int	 longitudIdArchivo=strlen(estructura->tempDestino)+1;
	t_estado_master estado=3;
	int		longitudMensaje=4;
	char* 	mensaje="aaa";
	char* buffer1 =malloc(longitudIdWorker + sizeof(int) + longitudIdArchivo + sizeof(t_estado_master) + sizeof(int) + longitudMensaje+1);
	desplazamiento = 0;
	...
	memcpy(buffer1+desplazamiento, estructura->tempDestino,longitudIdArchivo);
